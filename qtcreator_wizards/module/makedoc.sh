...
echo "QHP_NAMESPACE = \\"%{BundlePrefix}.%{QtModuleNameLower}.$verTag\\"" >> $doxyRes
echo "QHP_CUST_FILTER_NAME = \\"%{ModuleName} $version\\"" >> $doxyRes
echo "QHP_CUST_FILTER_ATTRS = \\"%{QtModuleNameLower} $version\\"" >> $doxyRes
echo "QHG_LOCATION = \\"$qtBins/qhelpgenerator\\"" >> $doxyRes
echo "INCLUDE_PATH += \\"$qtHeaders\\"" >> $doxyRes
echo "GENERATE_TAGFILE = \\"$destDir/%{QtModuleNameLower}/%{QtModuleNameLower}.tags\\"" >> $doxyRes
if [ "$DOXY_STYLE" ]; then
